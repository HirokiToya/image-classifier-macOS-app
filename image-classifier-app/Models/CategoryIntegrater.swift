...
class CategoryIntegrater {
    ...
    func integrateOnlyConsideringEvalution(categoryAttr: [CategoryAttribute], baseCategoryCount: Int, targetCategoryCount: Int) -> [CategoryAttribute] {
    ...
    func integrateConsideringSimilality(categoryAttr: [CategoryAttribute], baseCategoryCount: Int, targetCategoryCount: Int) -> [CategoryAttribute] {
        let caches = categoryCache.get(clusterNum: targetCategoryCount)
        if caches.count > 0 { return caches }
        else {
            var sceneCategoryCounts:[CategoryImagesCount] = []
